import { Copy, RotateCw, Check, X, Edit2, CheckCircle2, XCircle } from "lucide-react";
...
export default function MessageCard({ 
  ...
  return (
    <div className={`flex w-full group mb-6 ${isUser ? "justify-end" : "justify-start"}`}>
      <div className={`flex gap-4 w-full md:max-w-[75%] ${isUser ? "flex-row-reverse max-w-[85%]" : "max-w-full flex-row"}`}>
        ...
        <div className={`flex flex-col gap-1 min-w-0 ${isEditing ? 'w-full' : ''}`}>
           {/* Sender Name */}
           ...
           >
             ...
             {isEditing ? (
                <div className="flex flex-col gap-3">
                  <textarea
                    ...
                    className="w-full bg-black/20 border border-white/10 rounded-xl p-3 text-sm md:text-base text-foreground/90 resize-none focus:outline-none focus:border-primary/50 min-h-[60px]"
                    placeholder="Edit your message..."
                  />
                  <div className="flex justify-end gap-2">
                    <button
                      onClick={handleEditCancel}
                      className="px-3 py-1.5 text-xs font-medium bg-white/5 hover:bg-white/10 text-muted-foreground rounded-lg transition-colors flex items-center gap-1.5"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-medium bg-primary/20 hover:bg-primary/30 text-primary border border-primary/30 rounded-lg transition-colors flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Save & Submit
                    </button>
                  </div>
             ...
             {/* Action Buttons */}
             {!isEditing && (
               <div className={`absolute -bottom-4 ${isUser ? "left-2" : "right-2"} flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-background/80 backdrop-blur-md rounded-lg p-1 border border-white/10 shadow-lg`}>
                 {isUser ? (
                   <button 
